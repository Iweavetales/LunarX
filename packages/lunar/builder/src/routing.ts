...
export function BuildRouteNodeMap(entries: {
   ...
   */
  const routePatternToShardMap: { [routePattern: string]: BuiltShardInfo } = {}
  const serverSideShardsInRouteDir: { [routePattern: string]: BuiltShardInfo } =
    {} // 아래에서 라우트 맵을 완성 할 때 참조되는 맵 오브젝트

  for (let i = 0; i < entryPaths.length; i++) {
    const entryPath = entryPaths[i]
    const entryShard = entries[entryPath]
     ...
     * -> 라우트가 가능한 엔트리
     */
    const startedWithAppRoute = /^app\/routes/.test(entryPath)
    if (startedWithAppRoute) {
      if (entryShard.isServerSideShard) {
        ...
        const pathFromRouteRoot = entryPath.replace(/^app\/routes/, "")
        // server side script 라면 라우트 엔트리의 서버사이드 샤드일수 있으므로 관련처리
        const routePattern = convertEntryPathToRoutePatternPath(
          pathFromRouteRoot,
          true
        )
        ...
        // routable entry
        const routePattern =
          convertEntryPathToRoutePatternPath(pathFromRouteRoot)
  ...
  const routePatterns = Object.keys(routePatternToShardMap)
  /**
   * 항상 상위 라우팅은 미리 생성되어 있도록 순서를 정의하기 위해
   * routePath 길이가 작은 순서대로 정렬된 entryKey
   */
  const sortedRoutePatternsByLength = routePatterns.sort((a, b) => {
  ...
  for (let i = 0; i < sortedRoutePatternsByLength.length; i++) {
    const routePattern = sortedRoutePatternsByLength[i]
    const entryShard = routePatternToShardMap[routePattern]
    ...
    routeNodeMap[routePattern] = {
       ...
       */
      upperRoutePattern:
        upperRoutePattern === "/" ? undefined : upperRoutePattern,

      /**
       * 이 라우트에 해당하는 서버사이드 로더(somePage.server.ts) 샤드가 존재 한다면 해당 샤드의 entryPoint 를 입력
       */
      serverSideEntryPath: serverSideRouteLoaderShard
        ? serverSideRouteLoaderShard.entryPoint
        : undefined,
    }
  }
...
function FindUpperRoutePattern(
  ...
  //
  const routePatternTokens = routePattern.split("/")
