...
import { LoadBuiltShardEntryModule } from "./module-loader.ts"
 ...
 * 클라이언트에서 동작 할 소스코드가
 * AppStructureContext.ClientSourceFileMap 에 등록 될 때
...
export async function ResolveWebappStructure(
  ...
  for (let i = 0; i < entryKeys.length; i++) {
    ...
    if (entryPoint) {
      ...
      } else if (LibEntryFinder.LunarJSPlatformEntryServer.test(entryPoint)) {
        // app/lib/server
