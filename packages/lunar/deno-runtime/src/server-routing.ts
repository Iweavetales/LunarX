...
export function BuildRoutes(manifestRouteNodes: RawRouteInfoNodeMap): Node {
  ...
  root.add(
    "/static/*",
    async (req: Request, params: Params, swift: LunarServer) => {
  ...
  if (IsDevelopment()) {
    root.add(
      "/dev-helper/hmr",
      (req: Request, params: Params, swift: LunarServer) => {
  ...
  root.add(
    ...
    async (req: Request, params: Params, swift: LunarServer) => {
      ...
      const shardPath = shardUrl.substring(
        0,
        queryStartIndex === -1 ? shardUrl.length : queryStartIndex
      )
