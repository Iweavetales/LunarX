	...
	async getShardFile(shardPath: string): Promise<FsFile | null> {
		...
		if(nodeEnv !== "production" && /\.map$/.test(shardPath)){
			...
			return await Deno.open(
				this.getDistFilePath(shard.RealPath + '.map'),
				{
					read: true,
				},
			);
		...
		if (shard) {
			...
			const scriptFile = await Deno.open(
				this.getDistFilePath(shard.RealPath),
				{
					read: true,
				},
			);
	...
	async run() {
		...
		await httpServe(
			async (req) => {
				...
				const onlyPath = urlPath.substring(0, queryStartIndex === -1 ? urlPath.length:queryStartIndex)
