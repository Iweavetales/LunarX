...
type RequireFunction = (moduleNames: string[], callback: (modules: any[]) => void) => void;
type TAppData = {
  rd: {
  ...
  async function Startup() {
    ...
    async function prepareRouteShards(shardPaths: string[]) {
      ...
      for (let i = 0; i < loadedShards.length; i++) {
        ...
        if (loadedShardInfo && loadedShardInfo.shardPath && loadedShardInfo.module) {
          routeComponents[loadedShardInfo.shardPath] = loadedShardInfo.module.default;
        }
      }
    }
    await prepareRouteShards(ascRouteNodes.map(routeNode => routeNode.shardPath));
