...
export function BuildRouteNodeMap(entries: { [entryPath: string]: BuiltShardInfo }): RouteNodeMap {
  ...
  const serverSideShardsInRouteDir: { [routePattern: string]: BuiltShardInfo } = {}; // 아래에서 라우트 맵을 완성 할 때 참조되는 맵 오브젝트
  ...
  for (let i = 0; i < entryPaths.length; i++) {
    ...
    if (startedWithAppRoute) {
      if (entryShard.isServerSideShard) {
        ...
        const routePattern = convertEntryPathToRoutePatternPath(pathFromRouteRoot, true);
        ...
        // routable entry
        const routePattern = convertEntryPathToRoutePatternPath(pathFromRouteRoot);
